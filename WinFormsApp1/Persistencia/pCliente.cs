...
namespace WinFormsApp1.Persistencia
{
    internal class pCliente : IRepositorio<Cliente>
    {
        ...
        {
            Cliente cliente = new Cliente();
            SqliteCommand sqliteCommand = new SqliteCommand("SELECT id_cliente, nombre, apellido, nro_documento, telefono, email, localidad FROM CLIENTE WHERE id_cliente = @id");
