...
namespace WinFormsApp1.Persistencia
{
    internal class pReserva : IRepositorio<Reserva>
    {
        public List<Reserva> ObtenerTodos()
        {
            ...
            using (SqliteCommand sqliteCommand = new SqliteCommand("SELECT id_reserva, id_cliente, id_habitacion, id_usuario, fecha_entrada, fecha_salida, cantidad_personas, estado FROM RESERVA"))
            {
        ...
        {
            using (SqliteCommand sqliteCommand = new SqliteCommand("DELETE FROM RESERVA WHERE id_reserva = @Id"))
            {
                sqliteCommand.Parameters.Add(new SqliteParameter("@Id", id));
                sqliteCommand.Connection = Conexion.MiConexion;
                int rowsAffected = sqliteCommand.ExecuteNonQuery();
